...
def mujoco_creator2(type,x,y,z):
    blend_file_path = bpy.data.filepath
    directory = os.path.dirname(blend_file_path)
    new_dir = os.path.join(directory,"xml")
    ...
    xml_name = name + ".xml"
    xml_path = os.path.join(new_dir,xml_name)
...
def mujoco_creator(figures_db):
    ...
    for mesh_name, properties in figures_db.items():
        type_of_figure, x, y, z, volume, deviation = properties
...
def main():
    for obj in bpy.data.objects:
        ...
        if obj.type == "MESH" and not model_name.startswith("Cube" or "Cylinder" or "Sphere"):
            ...
            #mujoco_creator(option, mesh_name, dicc_of_figures[option][0], dicc_of_figures[option][1], dicc_of_figures[option][2])
...
for filename in os.listdir(blend_directory):
    if filename.endswith(".blend"):
        ...
        type_of_figure, dim_x, dim_y, dim_z, volume, deviation = main()

        figures_db[mesh_name] = [type_of_figure, dim_x, dim_y, dim_z, volume, deviation]

for mesh, details in figures_db.items():
    print(f"Mesh: {mesh}")
    print(f"  Figure: {details[0]}")
    print(f"  X: {details[1]}")
    print(f"  Y: {details[2]}")
    print(f"  Z: {details[3]}")
    print(f"  Volume: {details[4]}")
    print(f"  Deviation: {details[5]} ")
    print("\n")
mujoco_creator(figures_db)
